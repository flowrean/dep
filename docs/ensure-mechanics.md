...
The vendoring function is [`gps.WriteDepTree()`](https://godoc.org/github.com/flowrean/dep/gps#WriteDepTree). While it takes a handful of arguments, the relevant one is a [`gps.Lock`](https://godoc.org/github.com/flowrean/dep/gps#Lock) - an interface representing an abstracted form of the data held in a `Gopkg.lock`.
...
| (none)                               | (none)             | The first version that works, according to [the sort order](https://godoc.org/github.com/flowrean/dep/gps#SortForUpgrade) |
